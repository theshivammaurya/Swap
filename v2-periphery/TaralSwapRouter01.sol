...
contract TaralSwapRouter01 is ITaralSwapRouter01 {
    ...
    receive() external payable {
        assert(msg.sender == WTARAL); // only accept TARAL via fallback from the WTARAL contract
    }
    ...
    function _swap(uint[] memory amounts, address[] memory path, address _to) private {
        for (uint i; i < path.length - 1; i++) {
            ...
            ITaralSwapV2Pair(TaralSwapLibrary.pairFor(factory, input, output)).swap(amount0Out, amount1Out, to, new bytes(0));
        }
    }
